...
#'
#' @param HOBO The name of the datafile to be used in processing
...
ID_Inundation <- function(HOBO){
  ...
  Data <- HOBO # Creates a copy of data for processing

  # Determine at what time steps the nest is underwater
  classifyInundation <- NULL
  for(i in 1:length(Data$Inund_Severity)){
    if(is.na(Data$Inund_Severity[i])){ # Maintain NAs
      classifyInundation[i] <- NA
    } else if(Data$Inund_Severity[i] == 0){ # If Severity = 0, then no inundation event is currently underway
  ...
  if(sum(abs(inundationStartEnd), na.rm = TRUE) == 0){
    ...
    inundationStart <<- Data$Date_Time[which(inundationStartEnd == 1 | inundationStartEnd == 2)]
  }
  ...
  if(sum(abs(inundationStartEnd), na.rm = TRUE) == 0){
    ...
    inundationEnd <<-Data$Date_Time[which(inundationStartEnd == -1 | inundationStartEnd == 2)]
  }

  # Calculate the duration of individual inundation events
  inundationDuration <<- NULL
  if(is.POSIXct(inundationStart) == FALSE){
  ...
  if(is.POSIXct(inundationStart) == FALSE){
    ...
    for(i in 1:length(inundationStart)){
      ...
      inundationSeverity[i] <<- max(Data[which(Data$Date_Time >= inundationStart[i] &
                                                 Data$Date_Time <= inundationEnd[i]),]$Inund_Severity, na.rm = TRUE)
    }
  }
}

#' Umbrella function to clean depth data and identify inundation events including several QC checks using user input
#'
#' @param HOBO The name of the datafile to be used in processing
#' @return A new dataset (i.e., hoboData) containing calculated values based on the user input
#'
Clean_Depth_Data <- function(HOBO){
  ...
  ID_Inundation(HOBO = hoboData)

} # Cleans and transforms HOBO data relative to nest chamber and identifies inundation events

#' Example data

#' An example datafile reported from a HOBO U20L-04 water level logger deployed adjacent to a sea turtle nest ...
#'
#' @format ## `Demo_Data`
#' @name "hobaData"
